...
def plot_training_history(train_losses, train_accs, val_losses, val_accs):
    ...
    out_path = results_dir / "training_history_nn.png"
...
def main():
    """主函数"""
    if torch is None:
        print("错误: 未安装 torch，无法训练 DNN 模型。")
        print("建议: 使用 conda 安装 PyTorch，或按 PyTorch 官网指引安装对应版本。")
        return

    print("=" * 60)
    print("PUBG 武器声音分类 - 深度学习版本")
    print("=" * 60)
    ...
    hidden_dims = [128, 256, 128, 64]  # 多层神经网络

    model = WeaponClassifierNN(
        input_dim=input_dim,
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            best_model_path = models_dir / "best_model_nn.pth"
            torch.save({
                'epoch': epoch,
    ...
    models_dir = base_dir / "models"
    checkpoint = torch.load(models_dir / "best_model_nn.pth", map_location=device)
    ...
    plot_confusion_matrix(cm, le.classes_, 'Neural_Network')
    ...
    joblib.dump(le, models_dir / "label_encoder_nn.pkl")

    print(f"\n模型和预处理器已保存到: {models_dir}")
    print(f"最佳模型: {models_dir / 'best_model_nn.pth'}")
    print("\n训练完成！")
